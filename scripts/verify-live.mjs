...
const codePaths = [...html.matchAll(/(?:href|src)="((?:styles\.css|app\.js)(?:\?[^" ]*)?)"/g)].map(match => match[1]);
if (codePaths.length !== 2) throw new Error('Expected the published CSS and JS references');
const paths = ['index.html', ...codePaths, ...(await readdir(new URL('assets/', root))).map(name => `assets/${name}`)];
...
for (let i = 0; i < paths.length; i += 3) {
  rows.push(...await Promise.all(paths.slice(i, i + 3).map(async path => {
    ...
    const local = await readFile(new URL(path.split('?')[0], root));
